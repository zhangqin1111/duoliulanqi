(function attachTaskRouter(global) {
  const TASKS = {
    ...
    fact_check: {
      ...
      keywords: ['真假', '真实', '真实吗', '谣言', '爆料', '网传', '传闻', '截图', '视频', '这段话', '这张图', '可信', '辟谣'],
    },
    competitor_analysis: {
      label: '竞品对比',
      workflow: 'competitor_analysis_workflow',
      template: 'competitor_comparison_report',
      keywords: ['竞品', '对比', '哪个好', '区别', '差异', '功能', '价格', '选型', '替代', '相比', '优劣', '优势'],
    },
    consumer_purchase: {
      label: '消费选购决策',
    ...
    knowledge_brief: {
      ...
      keywords: ['是什么', '什么是', '为什么', '怎么理解', '如何理解', '原理', '解释', '科普', '总结', '梳理', '分析一下', '怎么看', '背景', '趋势'],
    },
    creative_content: {
      label: '内容创作方案',
      workflow: 'creative_content_workflow',
      template: 'creative_content_report',
      keywords: ['写', '文案', '脚本', '标题', '小红书', '短视频', '公众号', '海报', '广告', '营销', '方案', '策划', '改写', '润色'],
    },
    technical_diagnosis: {
      label: '技术诊断',
      workflow: 'technical_diagnosis_workflow',
      template: 'technical_diagnosis_report',
      keywords: ['代码', 'bug', '报错', '接口', '数据库', '架构', '部署', '性能', '前端', '后端', 'api', '服务器', '日志', '异常'],
    },
    learning_research: {
      label: '学习研究',
    ...
    career_recruiting: {
      ...
      keywords: ['简历', '面试', '岗位', '职业', '招聘', '跳槽', 'offer', '薪资', '求职', '绩效', 'okr', '述职'],
    },
    legal_risk: {
      label: '法律/合规初筛',
      workflow: 'legal_risk_screening_workflow',
      template: 'legal_risk_screening_report',
      keywords: ['合同', '条款', '违法', '违规', '合规', '法律', '侵权', '责任', '诉讼', '处罚', '律师', '风险条款'],
      highRisk: true,
    },
    medical_health: {
      label: '医疗健康初筛',
      ...
      keywords: ['症状', '体检', '报告单', '用药', '药', '医院', '医生', '疾病', '治疗', '诊断', '疼', '发烧', '咳嗽', '血压', '血糖'],
      highRisk: true,
    },
    finance_planning: {
      label: '金融规划初筛',
      ...
      keywords: ['理财', '基金', '保险', '贷款', '房贷', '信用卡', '还款', '资产配置', '收益', '亏损', '买入', '卖出', '投资建议'],
      highRisk: true,
    },
    general_compare: {
      label: '通用多源对比',
  ...
  function isGenericOpenEnded(question) {
    ...
    return /^(帮我)?(分析|看看|看下|研究|说说)(一下)?(这个|这件)?(事情|问题|内容)?$/.test(compact);
  }
