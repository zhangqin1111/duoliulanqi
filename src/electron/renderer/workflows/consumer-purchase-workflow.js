(function attachConsumerPurchaseWorkflow(global) {
  ...
  function buildRefinePrompt(rawQuestion, context) {
    ...
    return [
      ...
      '',
      '补全目标：后续系统要能产出明确机型对比、购买建议、适合人群、预算分层、价值权重、风险点、待核验价格/配置，而不是把所有判断都当成真假核验。',
      '',
      '硬性约束：',
      '1. 事实层必须严格：型号、版本、价格、配置、发布时间、售卖状态、官方参数必须要求模型标注是否可核验。',
      '2. 判断层允许裁决：性价比、值得买、适合谁，可以基于证据和用户场景做综合评分，不要因为它是价值判断就拒绝给建议。',
      hasDateHint
        ? '3. 用户给了明确时间或版本边界：必须沿用用户时间，不要自行扩大或替换时间范围。'
        : '3. 用户没有明确时间：按“当前/最近公开可买版本与主流渠道信息”理解，但不得编造具体日期。',
      '4. 必须要求模型输出：候选产品/同系列机型清单、价格/配置/定位、价值权重、首选/备选/不推荐、不同人群怎么选、风险与待核验项。',
      '5. 如涉及未发布、传闻、区域价格或促销价，必须标注“待核验”，不能写成确定事实；但也不能仅因回答中未附官网 URL 就直接判定产品不存在。',
      '6. 如果用户已经明确指定某一代产品或系列（如 iPhone 17），必须围绕该系列回答；除非有强证据证明该系列不存在，否则不要改写成上一代替代方案。',
